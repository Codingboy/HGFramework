...
while {true} do
{
	if (isServer) then
	{
		...
		_sync = _sync + 1;
		CODI_hgf_actualTime = [_year, _month, _day, _hour, _minute, _second];
		if (_sync == _syncF) then
		{
			publicVariable "CODI_hgf_actualTime";
			_sync = 0;
		};
	};
	waitUntil {!isNil "CODI_hgf_actualTime"};
	setDate[CODI_hgf_actualTime select 0, CODI_hgf_actualTime select 1, CODI_hgf_actualTime select 2, CODI_hgf_actualTime select 3, CODI_hgf_actualTime select 4];
