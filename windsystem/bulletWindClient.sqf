...
while {alive _bullet} do
{
	...
	_coriolisX = -0.02;
	if(_filter == 'magazine' || _filter == 'mgun') then
	{
		_bullet setVelocity [(_velX+((random _windX)/30))+_coriolisX, _velY + (random _windY)/30, _velZ + (random _windZ)/30];
	};
	if(_filter == 'grenadelauncher') then
	{
		_bullet setVelocity [(_velX+((random _windX)/80))+_coriolisX, _velY + ((random _windY)/80), _velZ + (random _windZ)/10];
	};
	
	sleep 0.01 + 1 - random gusts;
};
";
