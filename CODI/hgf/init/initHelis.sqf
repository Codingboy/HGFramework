...
if (isServer) then
{
	...
	sleep CODI_hgf_heliFirstDelay;
	while {_active} do
	{
		_paraPos = _markersPos select (floor(random(count _markersPos)));
		_para = "B_Parachute_02_F" createVehicle [_paraPos select 0, _paraPos select 1, CODI_hgf_heliDropHeight];
		...
		_para setPos [getPos _para select 0, getPos _para select 1, CODI_hgf_heliDropHeight];
		[_veh, _para] execVM "CODI\hgf\loot\vehicle\initVehicle.sqf";
		diag_log format ["%1 HGFramework: %2 spawned", time, _vehicle];
		sleep CODI_hgf_heliSpawnDelay;
	};
};