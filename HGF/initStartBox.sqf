...
{
	{
		...
		{
			_null = [_x] execVM "HGF\initCivilian.sqf";
	...
	{
		_null = [_pos] execVM "HGF\vasStart.sqf";
		waitUntil {scriptDone _null};
	}
	else
	{
		_null = [_pos] execVM "HGF\weaponBoxStart.sqf";
		waitUntil {scriptDone _null};
		_null = [_pos] execVM "HGF\weaponBoxStart.sqf";
		waitUntil {scriptDone _null};
		_null = [_pos] execVM "HGF\weaponBoxStart.sqf";
	...
	for "_i" from 0 to count hg_players-1 do
	{
		...
		_null = [[_coordX - hg_startRadius * (sin _ang) * _rnd, _y - hg_startRadius * (cos _ang) * _rnd, _z]] execVM "HGF\ammoBoxStart.sqf";
		waitUntil {scriptDone _null};
		
		_rnd = (random 0.25)+0.75-0.125;
		_ang = _angle + (random (360.0/count hg_players)) - (360.0/count hg_players/2);
		_null = [[_coordX - hg_startRadius * (sin _ang) * _rnd, _y - hg_startRadius * (cos _ang) * _rnd, _z]] execVM "HGF\supportBoxStart.sqf";
		waitUntil {scriptDone _null};
		
		//_rnd = (random 0.25)+1.25-0.125;
		//_ang = _angle + (random (360.0/count hg_players)) - (360.0/count hg_players/2);
		//_box = createVehicle["Box_NATO_AmmoOrd_F", [_coordX - hg_startRadius * (sin _ang) * _rnd, _y - hg_startRadius * (cos _ang) * _rnd, _z], [], 0, "NONE"];
		//_box setDir _ang+90;
		//_null = [_box, hg_crateRefillTime, hg_startCrateDespawnTime] execVM "HGF\explosivesBox.sqf";
		//waitUntil {scriptDone _null};
		
		//_rnd = (random 0.25)+1.5-0.125;
		//_ang = _angle + (random (360.0/count hg_players)) - (360.0/count hg_players/2);
		//_box = createVehicle["Box_NATO_Grenades_F", [_coordX - hg_startRadius * (sin _ang) * _rnd, _y - hg_startRadius * (cos _ang) * _rnd, _z], [], 0, "NONE"];
		//_box setDir _ang+90;
		//_null = [_box, hg_crateRefillTime, hg_startCrateDespawnTime] execVM "HGF\grenadeBox.sqf";
		//waitUntil {scriptDone _null};
		
		_rnd = (random 0.25)+1.25-0.125;
		_ang = _angle + (random (360.0/count hg_players)) - (360.0/count hg_players/2);
		_null = [[_coordX - hg_startRadius * (sin _ang) * _rnd, _y - hg_startRadius * (cos _ang) * _rnd, _z]] execVM "HGF\specialBoxStart.sqf";
		waitUntil {scriptDone _null};
